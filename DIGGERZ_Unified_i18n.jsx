...
function Responses(){const[data,setData]=useState([]);const[sel,setSel]=useState(null);const load=useCallback(()=>{try{setData(JSON.parse(localStorage.getItem("dgz_responses")||"[]"));}catch(e){setData([]);}},[]);
  ...
  return <div className="fu" style={{padding:"66px 16px 32px",maxWidth:1000,margin:"0 auto"}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:12,flexWrap:"wrap",gap:8}}><button onClick={()=>{const json=JSON.stringify(data,null,2);const b=new Blob([json],{type:"application/json"});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=`diggerz_responses_${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(u);}} style={{background:"none",border:`1px solid ${P.border}`,color:P.gray,padding:"6px 14px",cursor:"pointer",fontFamily:P.mono,fontSize:7,letterSpacing:"2px"}} disabled={data.length===0}>EXPORT JSON</button><button onClick={()=>{if(confirm("Effacer toutes les réponses ?")){localStorage.removeItem("dgz_responses");load();}}} style={{background:"none",border:`1px solid ${P.ruby}44`,color:P.ruby,padding:"6px 14px",cursor:"pointer",fontFamily:P.mono,fontSize:7,letterSpacing:"2px",opacity:0.6}} disabled={data.length===0}>CLEAR ALL</button></div>
  </div>;
}
